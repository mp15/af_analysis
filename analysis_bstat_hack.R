...
hist_plot <- ggplot(data=covdata, aes(x=ac_bin, y=snps, fill=coverage)) + geom_bar(position="dodge",stat="identity") + scale_fill_manual(values=palette) + scale_y_log10() + scale_x_discrete(limits = order, labels = names);
ggsave(filename=options[3],plot=hist_plot);
hist_indel_plot <- ggplot(data=covdata, aes(x=ac_bin, y=indel, fill=coverage)) + geom_bar(position="dodge",stat="identity") + scale_fill_manual(values=palette) + scale_y_log10() + scale_x_discrete(limits = order, labels = names);
