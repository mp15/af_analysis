...
thirty <- read.table(options[1], header=F, col.names=c(rep('',2),'ac_bin','snps','indel'), colClasses=c(rep('NULL',2),rep('numeric',3)));
thirty$coverage <- '30x';

fifteen <- read.table(options[2], header=F, col.names=c(rep('',2),'ac_bin','snps','indel'), colClasses=c(rep('NULL',2),rep('numeric',3)));
